...
test_that('histogram getAccuracy and getEpsilon return approximately correct values for stability mechanism', {
	...
	val2 <- round(histogram.getEpsilon(mechanism = 'mechanismStability', n.bins=20, n=2000, accuracy=0.5, delta=10^-6))
...
test_that('histogram getAccuracy and getEpsilon return approximately correct values for laplace mechanism', {
	...
	val2 <- round(histogram.getEpsilon(mechanism = 'mechanismLaplace', n.bins=20, n=2000, accuracy=0.5, delta=10^-6))
...
test_that('histogram with bins entered', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=4, n=my_n, epsilon=my_epsilon)
...
test_that('histogram release has expected dimensions and accuracy for logical variable with impute = false (laplace mechanism)', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=my_n.bins, n=my_n, epsilon=my_epsilon)
...
test_that('test on determineBins - ensure correct number of bins when bins are entered correctly', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=expected_number_of_bins, n=my_n, epsilon=my_epsilon)
    expect_equal(dp.histogram$epsilon, my_epsilon)
