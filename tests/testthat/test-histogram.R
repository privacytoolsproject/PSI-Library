...
test_that('histogram getAccuracy and getEpsilon return approximately correct values for stability mechanism', {
	val1 <- round(histogram.getAccuracy(mechanism = 'mechanismStability', n.bins=10, n=2000, epsilon=0.2, delta=10^-6, sensitivity=2))
	val2 <- round(histogram.getEpsilon(mechanism = 'mechanismStability', n.bins=20, n=2000, accuracy=0.5, delta=10^-6, sensitivity=2))
...
test_that('histogram with bins entered', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=4, n=my_n, epsilon=my_epsilon, sensitivity=2)
...
test_that('histogram release has expected dimensions and accuracy for logical variable with impute = false (laplace mechanism)', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=my_n.bins, n=my_n, epsilon=my_epsilon, sensitivity=2)
...
test_that('histogram on categorical data', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismStability', n.bins=length(catHistogram$result$release), n=my_n, epsilon=my_epsilon, delta=my_delta, sensitivity=2)
    expect_equal(catHistogram$epsilon, my_epsilon)
    expect_equal(catHistogram$accuracy, askAccuracy)
})
...
test_that('histogram has expected accuracy for stability mechanism', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismStability', n.bins=my_n.bins, n=my_n, epsilon=my_epsilon, delta=my_delta, sensitivity=2)
...
test_that('test on determineBins - ensure correct number of bins when bins are entered correctly', {
    ...
    askAccuracy <- histogram.getAccuracy(mechanism = 'mechanismLaplace', n.bins=expected_number_of_bins, n=my_n, epsilon=my_epsilon, sensitivity=2)
    expect_equal(dp.histogram$epsilon, my_epsilon)
