...
test_that('checkRange is as expected on 1D input', {
  rng1 <- c(0,1)
  rng2 <- c(0,1,2)
  rng3 <- c(1)
  ...
  #expect_warning(checkRange(rng2, "numeric"), expectedWarning)
  expect_equal(checkRange(rng2, "numeric"), c(0,2))
  checkRange(rng2, "numeric")
  ...
  #expect_error(checkRange(rng3, "numeric"), expectedError)
})

test_that('checkRange is as expected on 2D input', {
  ...
  expect_equal(checkRange(rng, 'numeric'),outRng)  #this is expected to throw warnings for both rows
})
