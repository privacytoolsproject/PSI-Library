...
test_that('error thrown when n not positive or whole number', {
    expect_error(dpGLM$new(mechanism='mechanismObjective', var.type='numeric', n=-1, epsilon=0.5, 
                           formula=form, rng=rng, objective='ols'),
                 "n must be a positive whole number")
    expect_error(dpGLM$new(mechanism='mechanismObjective', var.type='numeric', n=0.5, epsilon=0.5, 
                           formula=form, rng=rng, objective='ols'),
                 "n must be a positive whole number")
})
