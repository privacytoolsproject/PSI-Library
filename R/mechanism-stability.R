...
mechanismStability$methods(
    ...
    #' "x" using the Stability mechanism. In general, the Stability mechanism is meant to be used only with stable
    #' functions, which have a stronger guarantee on the output between neighboring databases. However, in this
    #' library, the Stability mechanism is optimized for the histogram statistic, and should only be used with 
    #' the histogram statistic. See references for more information on stable functions. 
    #' 
    #' The Stability mechanism should only be used on the histogram statistic in the PSIlence library.
    ...
    #' 
    #' The accuracy threshold for removing bins with low counts is calculated by: \eqn{1 + (2 * log(2 / \delta) / \epsilon)}.
    #' 
