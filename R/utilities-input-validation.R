...
checkEmpty <- function(n, emptyOkay=FALSE){
  ...
  if (isEmpty && !emptyOkay){
    stop(paste("Input may not be NA or NULL."))
  }
  else{
    return(n)
  }
...
checkFunction <- function(inFunc, allowedFuncs, type) {
  checkLength(inFunc, 1)
  checkEmpty(inFunc)
  checkVariableType(typeof('inFunc'), 'character')

  Func <- tolower(inFunc)
  indexFunc <- match(Func, tolower(allowedFuncs)) #get index of Func in allowedFuncs.
  # match returns NA if Func not in tolower(allowedFuncs).
  if (is.na(indexFunc)){
    stop(paste(type, 'Function', Func, 'should be one of', paste(allowedFuncs, collapse = ', ')))
  }
  return(allowedFuncs[indexFunc])
}
