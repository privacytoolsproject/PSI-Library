...
#' r_num <- histogram.release(x=x_num, var.type='numeric', n=100, epsilon=0.1, rng=c(-2, 2))
#' r_num_na <- histogram.release(x=x_num_na, var.type='numeric', n=100, epsilon=0.1, rng=c(-2, 2))
#' r_int <- histogram.release(x=x_int, var.type='integer', n=100, epsilon=0.1, rng=c(-40, 40))
#' r_num_random <- histogram.release(x=x_num, var.type='numeric', n=100, epsilon=0.1, rng=c(-2, 2))
#' # accuracy is returning inf, which filters the entire release for stability histogram
#' r_num_stability <- histogram.release(x=x_num, var.type='numeric', n=100, epsilon=0.1, rng=c(-2, 2))
#' r_num_noisy <- histogram.release(x=x_num, var.type='numeric', n=100, epsilon=0.1, rng=c(-2, 2))
#'
#' # categorical types
#' x_char <- c(rep('a', 40), rep('b', 25), rep('c', 15), rep('d', 12), rep('e', 5), rep('f', 2), rep('g', 1))
#' x_fac <- factor(x_char)
#' bins <- c('a', 'b', 'c', 'd', 'e')
#' r_char <- histogram.release(x=x_char, var.type='character', n=100, epsilon=0.1, bins=bins)
#' r_fac <- histogram.release(x=x_fac, var.type='factor', n=100, epsilon=0.1, bins=bins)
