...
#' r_num_random <- histogram.release(x_num, var.type='numeric', range=c(-2, 2), n=100, 
#'    epsilon=0.1, mechanism='random')
...
#' r_num_noisy <- histogram.release(x_num, var.type='numeric', range=c(-2, 2), n=100, 
#'    epsilon=0.1, mechanism='noisy')
#'
#' # categorical types
#' x_char <- c(rep('a', 40), rep('b', 25), rep('c', 15), rep('d', 12), rep('e', 5), 
#'    rep('f', 2), rep('g', 1))
#' x_fac <- factor(x_char)
#' bins <- c('a', 'b', 'c', 'd', 'e')
