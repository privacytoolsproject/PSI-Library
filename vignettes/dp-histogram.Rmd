...
Similar to logical variables, you can specify whether or not you would like NA values in the data to be imputed, using the `impute` parameter. If `impute = FALSE` and there are `NA` values in the underlying data, then an `NA` bin will be added to the histogram. This can be seen above, because __the default value for `impute` is `FALSE`__. If `impute = TRUE`, then there will not be an `NA` bucket in the final histogram, and any existing `NA` values will be replaced by values in the data.

```{r}
characterHistogram <- dpHistogram(varType='character', variable="Employee.Status", n=469, epsilon=0.5,
                                   bins=c("Detailee","Employee","Employee (part-time)"), impute = TRUE)
characterHistogram$release(SocrataWhiteHouseEmployeeSalaries)
print(characterHistogram$result)
```
...
characterHistogram <- dpHistogram(varType='character', variable="Employee.Status", n=469, epsilon=0.5,
                                  bins=c("Detailee","Employee","Employee (part-time)"),
                                  imputeBins=c("Detailee","Employee"), impute = TRUE)
characterHistogram$release(SocrataWhiteHouseEmployeeSalaries)
print(characterHistogram$result)
```
...
* `variance` \ The variance of the noisy data. Only available for logical vectors.
* `stdDev` \ The standard deviation of the noisy data. Only available for logical vectors.
