...
* `rng` \ Numeric, a 2-tuple with an estimate of the lower and upper bounds of the data as determined by the user. In other words, it should be a vector in the form: c(lower bound, upper bound). Ignored for `'character'` and `'logical'` data types. Optional for `numeric` and `integer` types, default `NULL`. If `NULL` and `bins` also `NULL`, the stability mechanism will be used to calculate the histogram (see ?stabilityMechanism).
...
The `release` method calculates the differentially private histogram of given data and accepts a single argument:
...
To calculate the histogram of a logical variable, input a logical vector into `variable` and update `varType` to `'logical'`. Also add a boolean flag for `impute` to indicate if you would like empty values to be imputed from the existing data. (If `impute = TRUE`, then there will only be two bins in the histogram: 0 and 1. If `impute = FALSE`, then there will be three bins in the histogram: 0, 1, and NA.) Note: you do not need to enter a range for a logial variable.
...
For logical variables, it is never necessary to enter a vector of bins, you just need to specify whether to impute empty values or not. It is also not necessary to enter a range for logical variables, because the values of a logical variable are known to be {0,1}.
...
If you would like NA (empty or unknown) values to be imputed (or replaced) by a specific subset of input bins, you can specify the imputation bins with `imputeBins`. You may want to do this if you know the underlying data is skewed, meaning an unkown value is more likely to be of a certain value as opposed to others. Specifying `imputeBins` is optional, and if it is not specified the NA values will be imputed using a uniform distribution on all imput bins (i.e. NA values will be replaced with any known value with equal probability).
