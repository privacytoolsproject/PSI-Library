...
The `dpGLM` class produces fits for generalized linear models using a perturbed objective function. Currently, the class supports fits for continuous dependent variables via `'ols'`, count variables via `'poisson'` objective, and binary variables via either `'logit'` or `'probit'`. 
...
The minimal example below demonstrates a linear regression model fit via `dpGLM` using the `PUMS5extract10000` data. In this case, we fit a model of income as a linear function of education level and gender. First attach the data and specify the necessary metadata.
...
rng <- matrix(c(1, 16, 0, 1), ncol=2, byrow=TRUE)
...
* `rng` \ Numeric, a $p \times 2$ matrix where each row is a 2-tuple giving an a priori estimate of the lower and upper bounds of the $p^{\text{th}}$ variable in the data frame.
...
To fit a linear regression model via `dpGLM`.
