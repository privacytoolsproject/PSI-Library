...
example_dpVariance <- dpVariance$new(mechanism='mechanismLaplace', var.type='numeric', 
                      variable='x1', epsilon=10, n=10, rng=c(0, 70))
out1 <- example_dpVariance$release(data)
print(out1)

example_dpVariance2 <- dpMean$new(mechanism='mechanismLaplace', var.type='logical', 
...
* `data` \ Data frame containing numeric columns corresponding the names specified in `formula`.
...
```{r}
numeric_variance <- dpVariance$new(mechanism='mechanismLaplace', var.type='numeric', 
...
To calculate the variance of a logical vector instead, input a logical vector into `variable` and update `var.type` and `rng` appropriately:

```{r}
logical_variance <- dpVariance$new(mechanism='mechanismLaplace', var.type='logical', 
