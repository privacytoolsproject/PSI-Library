...
# release variance on income
dp.variance <- dpVariance$new(mechanism='mechanismSnapping', var.type='numeric',
                      variable='income', epsilon=0.1, n=nrow(PUMS5extract10000), rng=c(0, 200000))
dp.variance$release(PUMS5extract10000)
print(dp.variance$result)

# release covariance of income and sex
ranges <- do.call(rbind, list(c(0,1), c(0,1)))
dp.covariance <- dpCovariance$new(mechanism = 'mechanismSnapping', var.type = 'logical',
                                 n = nrow(PUMS5extract10000), epsilon = 0.1,
                                 columns = c('sex', 'married'), rng = ranges)
dp.covariance$release(PUMS5extract10000)
print(dp.covariance$result)

ranges <- do.call(rbind, list(c(0,1), c(0,1)))
dp.covariance2 <- dpCovariance$new(mechanism = 'mechanismLaplace', var.type = 'logical',
                                 n = nrow(PUMS5extract10000), epsilon = 0.1,
                                 columns = c('sex', 'married'), rng = ranges)
dp.covariance2$release(PUMS5extract10000)
print(dp.covariance2$result)
```