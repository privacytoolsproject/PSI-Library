...
did not satisfy the formal privacy guarantees because of error introduced by the floating-point implementation. We provude a high-level overview directly
below, but the interested reader can consult the [Further Information on Vulnerability] section for a slightly more technical explanation.

The Laplace Mechanism is theoretically an $$(\epsilon, 0)$$-DP algorithm, but can become an $$(\epsilon', \delta)$$-DP algorithm
in practice (when implemented in software) where we know $$\epsilon' > \epsilon$$ and $$\delta > 0$$, but don't have exact guarantees on either.
...
Snapping Mechanism vs. Laplace Mechanism
----------------------------------------

Further Information on Vulnerability
------------------------------------
...
dp.mean <- dpMean$new(mechanism='mechanismSnapping', var.type='numeric',
                      variable='income', epsilon=0.1, n=nrow(PUMS5extract10000), rng=c(0, 200000))
dp.mean$release(PUMS5extract10000)
print(dp.mean$result)

# release mean on sex
dp.mean2 <- dpMean$new(mechanism='mechanismSnapping', var.type='logical',
